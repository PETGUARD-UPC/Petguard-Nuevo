...
public interface KeeperRepository extends JpaRepository<Keeper, Integer>{
	...
	@Query(value = "SELECT k.name,count(ide.id_reserve) from public.reserves i join reservedetails ide on  ide.id_reserve = i.id_reserve join keepers k on k.id_keeper= ide.id_keeper group by k.name ORDER BY COUNT(k.name) DESC limit 5", nativeQuery = true)
